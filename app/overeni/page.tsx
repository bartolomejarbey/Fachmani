    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-28 pb-12">
        ...
        {alreadyVerifiedAres && !profile?.is_verified && (
          <div className="bg-amber-50 border border-amber-300 rounded-2xl p-5 mb-6 text-sm text-amber-900 space-y-3">
            <div>
              <strong>✓ Krok 1/2 hotov:</strong> ARES ověření bylo úspěšné
              (<strong>{profile?.ares_verified_name}</strong>).
            </div>
            <div className="border-t border-amber-200 pt-3">
              <strong>Krok 2/2 — Ověření bankovního účtu:</strong> Pro dokončení
              je nutné poslat <strong>symbolickou 1 Kč platbu</strong> z vašeho
              podnikatelského účtu na náš účet. Tím prokážete, že podnikatelský
              účet, který uvádíte, opravdu patří vám.
            </div>
            <div className="text-xs text-amber-800">
              V profilu zadáte číslo účtu, my vygenerujeme variabilní symbol
              a QR kód. Po obdržení platby admin ověření finálně potvrdí
              (typicky do 24 h).
            </div>
            <Link
              href="/dashboard/profil#bank-verification"
              className="inline-flex items-center gap-2 bg-amber-600 hover:bg-amber-700 text-white font-semibold px-4 py-2 rounded-xl shadow-sm transition mt-1"
            >
              Pokračovat na krok 2 (1 Kč platba) →
            </Link>
          </div>
        )}
