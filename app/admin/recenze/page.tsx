...
export default function AdminRecenze() {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      .from("reviews")
      .select(
        "id, rating, rating_quality, rating_communication, rating_price, comment, created_at, provider_id, customer_id, provider:provider_id(full_name), customer:customer_id(full_name, email)"
      )
    ...
    type RawRow = Omit<Review, "provider_name" | "customer_name" | "customer_email"> & {
      provider: { full_name: string | null } | null;
      customer: { full_name: string | null; email: string | null } | null;
    };

    const rows: Review[] = ((data as unknown as RawRow[] | null) ?? []).map((r) => ({
      id: r.id,
      rating: r.rating,
  ...
  const handleDelete = async (id: string, providerName: string | undefined) => {
    if (!confirm(`Opravdu smazat tuto recenzi pro ${providerName || "fachmana"}?`)) return;
    ...
    await supabase.from("admin_activity_log").insert({
      ...
      target_type: "review",
      target_id: id,
      details: { provider_name: providerName },
    });

    setMessage("✅ Recenze smazána.");
    await load();
  };
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-white">⭐ Recenze</h1>
          <p className="text-slate-400">
            Celkem {counts.total} recenzí · {counts.low} nízkých (≤2★) · {counts.high} extrémně vysokých (5★) · {counts.noComment} bez komentáře · {counts.recent} za poslední týden
          </p>
        </div>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        ...
                        {r.rating_quality !== null && (
                          <span className="text-xs text-slate-500">
                            (Q: {r.rating_quality} · K: {r.rating_communication} · C: {r.rating_price})
                          </span>
                        )}
                        <span className="text-xs text-slate-500">
                    ...
                    <div className="flex gap-2 shrink-0">
                      <Link
                        ...
                        title="Zobrazit profil fachmana"
                      >
                        👁️ Profil
                      </Link>
                      <button
                        onClick={() => handleDelete(r.id, r.provider_name)}
